...
export default function SearchFilterBar({ className = "", searchValue, onSearchChange }) {
    return (
        <div className={`bg-dark-800 p-5 py-6 rounded-xl w-full ${className}`}>
            {/* Desktop layout */}
            <div className="hidden md:flex items-center justify-between w-full">
                <div className="flex gap-6 max-w-[1050px] w-full">
                    <div className="flex items-center bg-transparent border border-light/50 rounded-lg px-4 py-2 h-12 flex-2 w-[350px]">
            ...
            <div className="flex flex-col gap-4 md:hidden w-full">
                <div className="flex items-center bg-transparent border border-light/50 rounded-lg px-4 py-2 h-12 w-full">
                    <Search className="text-light/50 mr-3 w-5 h-5 flex-shrink-0" />
                    <input
                        type="text"
                        value={searchValue}
                ...
                <select className="bg-transparent border border-light/50 rounded-lg px-4 py-2 h-12 text-light/50 w-full">
                    <option>All Sizers</option>
                </select>
                <select className="bg-transparent border border-light/50 rounded-lg px-4 py-2 h-12 text-light/50 w-full">
                    <option>All Time</option>
                ...
                <button className="bg-light/5 border-none rounded-lg w-full h-12 flex items-center justify-center self-end">
                    <RotateCcw className="text-light/40 w-full h-5" />
                </button>
            </div>
        </div>
    );
